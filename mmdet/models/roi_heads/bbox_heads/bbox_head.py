...
class BBoxHead(BaseModule):
    ...
    ### OSODD
    @force_fp32(apply_to=('cls_score', 'bbox_pred', 'penultimate_feat'))
    def get_bboxes_osodd(self,
                   rois,
                   cls_score,
                   bbox_pred,
                   penultimate_feat,
        ...
        #print("penultimate_feat:", penultimate_feat.shape)

        if cfg is None:
            return bboxes, scores, cls_score, penultimate_feat
        else:
            det_bboxes, det_labels, inds = multiclass_nms(bboxes, scores,
                                                    cfg.score_thr, cfg.nms,
                                                    cfg.max_per_img, return_inds=True)
            # inds returned from nms are relative to (num_proposals * num_classes) indexing, not num_proposals
            orig_inds = inds // (cls_score.shape[1] - 1)
            det_logits = cls_score[orig_inds]
            det_penultimate_feats = penultimate_feat[orig_inds]
            #print("\nPost-NMS...")
            #print("inds:", inds, inds.shape, inds.dtype)
            #print("orig_inds:", orig_inds, orig_inds.shape, orig_inds.dtype)
            #print("det_bboxes:", det_bboxes, det_bboxes.shape)
            #print("det_labels:", det_labels, det_labels.shape)
            #print("det_logits:", det_logits, det_logits.shape)
            #print("det_penultimate_feats:", det_penultimate_feats, det_penultimate_feats.shape)
            #print("sanity check.. scores[orig_inds]:", scores[orig_inds], scores[orig_inds].shape)
            #print("sanity check.. bboxes[orig_inds]:", bboxes[orig_inds].view(3, -1, 4), bboxes[orig_inds].view(3, -1, 4).shape)
            return det_bboxes, det_labels, det_logits, det_penultimate_feats
