_base_ = [
    '../../_base_/datasets/osodd/coco365.py',  # CHECK
...
lr_config = dict(
    ...
    warmup_ratio=0.001,
    step=[8, 11])  # CHECK
runner = dict(type='EpochBasedRunner', max_epochs=12)  # CHECK


##########################################################################
### RUNTIME
##########################################################################
checkpoint_config = dict(interval=4)  # CHECK

load_from = None
resume_from = None
workflow = [('train', 1)]

work_dir = 'out/osodd/coco365/faster_rcnn_r50_oln_fpn'  # CHECK
